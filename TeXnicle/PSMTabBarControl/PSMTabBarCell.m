...
- (void)setIsInOverflowMenu:(BOOL)value
{
	if (_isInOverflowMenu != value) {
		...
		if ([[self controlView] respondsToSelector:@selector(delegate)]) {
			id delegate = [[self controlView] performSelector:@selector(delegate)];
			if ([delegate respondsToSelector:@selector(tabView:tabViewItem:isInOverflowMenu:)]) {
				[delegate tabView:(NSTabView*)[self controlView] tabViewItem:[self representedObject] isInOverflowMenu:_isInOverflowMenu];
			}
		}
	}
}
